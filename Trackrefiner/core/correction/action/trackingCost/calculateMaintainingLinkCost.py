...
def calc_maintain_exist_link_cost(sel_source_bacteria_info, sel_target_bacteria_info, center_coord_cols,
    ...
    if sel_target_bacteria_info.shape[0] > 0 and sel_source_bacteria_info.shape[0] > 0:
        ...
        feature_list = ['iou', 'min_distance', 'neighbor_ratio', 'Direction_of_Motion',
                        'Motion_Alignment_Angle', 'Length_Change_Ratio']
        ...
        division_merged_df['adjusted_Neighbor_Shared_Count_daughter'] = np.where(
            ...
            division_merged_df['Neighbor_Shared_Count_daughter'] + 1,
            division_merged_df['Neighbor_Shared_Count_daughter']
        )

        division_merged_df['neighbor_ratio_daughter'] = \
            (division_merged_df['Neighbor_Difference_Count_daughter'] / (
                division_merged_df['adjusted_Neighbor_Shared_Count_daughter']))

        continuity_bac_merged_df['adjusted_Neighbor_Shared_Count_bac2'] = np.where(
            continuity_bac_merged_df['Neighbor_Shared_Count_bac2'] == 0,
            continuity_bac_merged_df['Neighbor_Shared_Count_bac2'] + 1,
            continuity_bac_merged_df['Neighbor_Shared_Count_bac2']
        )

        continuity_bac_merged_df['neighbor_ratio_bac2'] = \
            (continuity_bac_merged_df['Neighbor_Difference_Count_bac2'] / (
                continuity_bac_merged_df['adjusted_Neighbor_Shared_Count_bac2']))
        ...
        continuity_bac_merged_df = continuity_bac_merged_df.rename(
            {'Neighbor_Difference_Count_bac2': 'Neighbor_Difference_Count',
             'neighbor_ratio_bac2': 'neighbor_ratio',
             'Direction_of_Motion_bac2': 'Direction_of_Motion',
             'Motion_Alignment_Angle_bac2': 'Motion_Alignment_Angle',
             'Length_Change_Ratio_bac2': 'Length_Change_Ratio'}, axis=1)
