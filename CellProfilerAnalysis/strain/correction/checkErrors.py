...
def check_errors_for_without_parent_link(df, all_bacteria_in_candidate_time_step, candidate_time_step,
                                         min_life_history_of_bacteria, transition_time_step):

    bacteria_time_step_under_invest = \
        all_bacteria_in_candidate_time_step.loc[(all_bacteria_in_candidate_time_step['division_time'] == 0) |
                                                (all_bacteria_in_candidate_time_step['division_time'] >=
                                                 candidate_time_step + min_life_history_of_bacteria)]

    bacteria_time_step_under_invest = bacteria_time_step_under_invest.copy()
    bacteria_time_step_under_invest['pass'] = True
    bacteria_time_step_under_invest['continued_life_history_before_transition'] = False

    max_daughter_len_to_mother_ratio_boundary = find_max_daughter_len_to_mother_ratio_boundary(df)

    for bac_indx, bacterium in bacteria_time_step_under_invest.iterrows():

        # before the birth of the transition bacterium or at the same time
        candidate_bac_life_history_before_transition = df.loc[(df['id'] == bacterium['id']) &
                                                              (df['ImageNumber'] <= transition_time_step) &
                                                              (df['ImageNumber'] > candidate_time_step)]

        if candidate_bac_life_history_before_transition.shape[0] > 0:

            # This means that a division may have occurred (maybe the transition bacterium is the second daughter or
            # the division occurred before transition bacterium, but segmentation errors occurred).

            length_ratio = candidate_bac_life_history_before_transition['AreaShape_MajorAxisLength'].iloc[0] / \
                           bacterium['AreaShape_MajorAxisLength']

            if length_ratio > 1 or length_ratio > \
                    max_daughter_len_to_mother_ratio_boundary['avg'] + \
                    1.96 * max_daughter_len_to_mother_ratio_boundary['std']:

                bacteria_time_step_under_invest.at[bac_indx, 'pass'] = False
            else:
                bacteria_time_step_under_invest.at[bac_indx, 'continued_life_history_before_transition'] = True

    bacteria_time_step_under_invest = \
        bacteria_time_step_under_invest.loc[bacteria_time_step_under_invest['pass'] == True]

    return bacteria_time_step_under_invest
