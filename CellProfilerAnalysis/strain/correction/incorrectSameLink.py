...
from CellProfilerAnalysis.strain.correction.action.compareBacteria import (division_detection_cost,
                                                                           adding_new_link_cost,
                                                                           make_initial_distance_matrix, calc_maintenance_cost)
...
def assign_new_link(df, neighbors_df, source_bac_index, source_bac, division_cost_df, new_link_cost_df,
                    all_bac_in_target_time_step, maintenance_cost):
        ...
        if (source_bac_division_cost['Cost'].values.tolist()[0] != 999 or
            ...
            if source_bac_division_cost['Cost'].values.tolist()[0] < source_bac_new_link_cost['Cost'].values.tolist()[
                ...
                if maintenance_cost_this_link > source_bac_new_link_cost['Cost'].values.tolist()[0]:
                    df = new_transition_bacteria(df, incorrect_link_life_history)
...
def incorrect_same_link(df, neighbors_df, sorted_npy_files_list, min_life_history_of_bacteria, interval_time, logs_df):
