...
class TrackingGUI(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Bacterial Tracking Visualization")
        ...
        self.btn_open_file = tk.Button(self.open_file_frame, text="Select Trackrefiner CSV Output File",
    ...
    def correction_tracking(self):
        ...
        new_window.title("Trackrefiner Tracking Correction")
    ...
    def display_images(self, timestep1, timestep2, specified_object_numbers, specified_ids_str,
        ...
        df_t1 = df[df['stepNum'] == int(timestep1)].reset_index(drop=True)
        df_t2 = df[df['stepNum'] == int(timestep2)].reset_index(drop=True)
    ...
    def show_image(self, df_current, image_path, window_title, specified_object_numbers=None, specified_ids=None,
        ...
        # Extract objects and plot as per the logic from your existing script
        ...
        if specified_object_numbers or specified_ids or specified_parent_ids:
            ...
            if neighbor_distance:
                ...
                dist_cond = (
                        np.sqrt(np.power((merged_df[center_cols['x'] + '_1'] -
                                          merged_df[center_cols['x'] + '_2']), 2) +
                                np.power((merged_df[center_cols['y'] + '_1'] -
                                          merged_df[center_cols['y'] + '_2']), 2)) <= neighbor_distance)

                neighbors_bac_id = merged_df[dist_cond]['id_2'].values.tolist()
                specified_ids.extend(neighbors_bac_id)

        # Filter objects by ID, Parent ID, or distance
        print("===================== Time Step: " + str(df_current['stepNum'].values[0]) + '=====================')
    ...
    def show_all_time_steps(self):
        ...
        def update_image(val):
            ...
            df_current = df[df['stepNum'] == timestep].reset_index(drop=True)
...
# Function to extract bacteria information
...
# Function to check for coordinate columns
def checking_columns(data_frame):
    if 'Location_Center_X' in data_frame.columns and 'Location_Center_Y' in data_frame.columns:
        return {'x': 'Location_Center_X', 'y': 'Location_Center_Y'}
    elif 'AreaShape_Center_X' in data_frame.columns and 'AreaShape_Center_Y' in data_frame.columns:
        return {'x': 'AreaShape_Center_X', 'y': 'AreaShape_Center_Y'}
    else:
        raise ValueError("Center coordinates not found in data")


# Function to find vertex as per your logic
def find_vertex(center_x, center_y, major, angle_rotation, angle_tolerance=1e-6):
    if np.abs(np.abs(angle_rotation) - np.pi / 2) < angle_tolerance:
        ...
        semi_major = major
        vertex_1_x = float(semi_major / (np.cos(angle_rotation) + np.tan(angle_rotation) * np.sin(angle_rotation)) + center_x)
        vertex_1_y = float((vertex_1_x - center_x) * np.tan(angle_rotation) + center_y)
        vertex_2_x = float(-semi_major / (np.cos(angle_rotation) + np.tan(angle_rotation) * np.sin(angle_rotation)) + center_x)
